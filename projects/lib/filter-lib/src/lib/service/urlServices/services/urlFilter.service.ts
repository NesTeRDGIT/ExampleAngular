...
/** Сервис преобразования данных фильтрации в URL */
...
export class UrlFilterService {
  ...
  toUrlParameters = (filterList: FilterField[]): UrlParameter[] => {
    const filterFields = filterList.filter(x => x.hasValue && !x.singleUrl);
    const singleUrlFields = filterList.filter(x => x.singleUrl);
    ...
    singleUrlFields.forEach(x => {
      if(x.field.length>0){
        result.push({ name: x.field[0], value: this.getValue(x, false) });
      }
    });
    return result;
  }
  ...
  private valueToString = (value: unknown, quotesString = true): string => {
    ...
    if (value instanceof DateType) {
      if (value.sendTime) {
        return value.ToDateStringWithoutTimeZone();
      } else {
    ...
    throw new InvalidCastException(value, 'string');
  }
